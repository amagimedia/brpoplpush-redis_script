...
RSpec.describe Brpoplpush::RedisScript::Client, "#execute" do
  ...
  let(:script)              { Brpoplpush::RedisScript::Script.new(name: script_name, root_path: SCRIPTS_PATH) }
  let(:scripts)             { instance_spy(Brpoplpush::RedisScript::Scripts) }
  ...
  context "when error is raised" do
    before do
      ...
      allow(scripts).to receive(:execute) do
        call_count += 1
        call_count.odd? ? raise(Redis::CommandError, redis_error_message) : "bogus"
      end
    ...
    context "when message starts with BUSY" do
      ...
      context "when .script(:kill) raises CommandError" do
        before do
          allow(scripts).to receive(:kill).and_raise(Redis::CommandError, "NOT BUSY")
          allow(client.logger).to receive(:warn)
        end

        specify do
          expect { execute }.not_to raise_error
          expect(client.logger).to have_received(:warn).with(kind_of(Redis::CommandError))
